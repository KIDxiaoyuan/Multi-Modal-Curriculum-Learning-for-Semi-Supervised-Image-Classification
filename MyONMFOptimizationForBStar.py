...
def MyONMFOptimizationForBStar(B1, B2, B_Star, Sigma):
    # User Settings
    C = 1.2
    ...
    MaxIter = 100
    # =====================================
    Lembda = np.zeros(B_Star.shape)
    stepB = 1
    error = np.zeros(MaxIter)
    converged = False
    Iter = 0
    T = np.maximum(0, B_Star + Lembda / Sigma)
    while not converged:
        Iter = Iter + 1
        # update B
        B_New, stepB, Fvalue = updateB(B1, B2, B_Star, Sigma, T, Lembda, stepB, minOpts_maxMinIters,
                                       lsOpts_stepBeta, lsOpts_stepLowerThreshold)
        # stop Criterion
        error[Iter] = np.linalg.norm(B_New - B_Star, ord='fro')
        if ((error[Iter] / np.linalg.norm(B_Star, ord='fro')) < tol) or (Iter == MaxIter):
            converged = True
        # update parameters
...
def updateB(B1, B2, B, Sigma, T, Lembda, step, minOpts_maxMinIters, lsOpts_stepBeta,
            lsOpts_stepLowerThreshold):
    agrad = lambda B: np.dot(Sigma, B - T) + Lembda + np.dot(2, np.dot(2, B) - B1 - B2)
    stepMove = lambda B, step: stepMoveB(B, step, agrad)
    ObjFunValue = lambda B: ComputeLagrangianObjFunValue(B1, B2, B, Sigma, T, Lembda)
...
def ComputeLagrangianObjFunValue(B1, B2, B, Sigma, T, Lembda):
    BMinusT = B - T
    G1 = np.sum(np.einsum('ij,ij->j', B - B1, B - B1) + np.einsum('ij,ij->j', B - B2, B - B2), axis=0)
    G2 = np.sum(np.einsum('ij,ij->j', Lembda, BMinusT), axis=0)
    G3 = np.dot(np.dot(0.5, Sigma), np.sum(np.einsum('ij,ij->j', BMinusT, BMinusT), axis=0))
